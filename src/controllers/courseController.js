...
export const createCourse = async (req, res) => {
  try {
    ...
    // كورس جديد اتضاف — أي صفحة كتالوج محفوظة في الكاش بقت قديمة، لازم تتمسح
...
// ⚡ OPTIMIZED GET ALL COURSES — كتالوج عام، بيتقرا كتير جدًا وبيتغيّر نادرًا،
// أفضل مرشّح للكاش في المنصة كلها
export const getAllCourses = async (req, res) => {
  try {
    const page = Math.max(1, parseInt(req.query.page) || 1);
    const limit = Math.min(100, Math.max(1, parseInt(req.query.limit) || 50));
    ...
    // جلب الحقول الأساسية للكروت فقط لتسريع الاستجابة وتقليل حجم الداتا
    const courses = await Course.find()
      .select("title price thumbnail category instructorName")
      .skip(skip)
      .limit(limit)
      .lean();

    const payload = { courses, pagination: { page, limit } };

    // 60 ثانية كفاية لكتالوج بيتغيّر نادرًا، وبتمنع أي بيانات قديمة تفضل ظاهرة كتير
...
export const deleteCourse = async (req, res) => {
  try {
    ...
    // الكورس اتمسح — لازم نمسح نسخته من الكاش وكل صفحات الكتالوج المحفوظة
    await Promise.all([
      cacheDelPattern(`courses:detail:${id}`),
      cacheDelPattern("courses:list:*"),
    ]);

    res.status(200).json({ deletedCourse, message: "Course has been deleted" });
  } catch (error) {
    return res
      .status(500)
      .json({ message: "Server Error", error: error.message });
...
export const updateCourse = async (req, res) => {
  try {
    ...
    // الكورس اتعدّل — نفس منطق الحذف، لازم نمسح النسخة القديمة من الكاش
    await Promise.all([
      cacheDelPattern(`courses:detail:${id}`),
      cacheDelPattern("courses:list:*"),
    ]);

    res.status(200).json({ updatedCourse, message: "Course has been updated" });
  } catch (error) {
    return res
      .status(500)
      .json({ message: "Server Error", error: error.message });
